...
| **Persona lens, not character** | **no `The {Persona}` token appears in body or callouts. The persona is the lens, never a named speaker inside the article. Foils to other personas use lowercase generic labels (`hawks will argue`, not `The Hawk will argue`).** | `persona_self_ref: "{offending phrase}" names the persona inside the article` |
| At least one rival-challengeable claim | self-check | `no_rival_hook` |
| No double-apostrophe artifacts | no literal `''` sequences anywhere in body/callouts/sources/headline/subheadline/summary/hero_image_caption. Dollar-quoted `$json$...$json$` SQL blocks do not need `''` escaping; regular `'...'` string literals do. | `double_apos: {column} contains literal ''` |
| Unique hero image | hero_image_url points to a file unique to this slug, even when the article shares a `story_group_id` with other articles in a batch | `hero_reused: image matches another article's hero` |
...
**The persona-lens gate is the second most often-broken one.** After shipping a batch of articles on 2026-04-11 where every piece named its assigned persona inside the body ("The Populist follows the money", "The Futurist grants the curve", "The Prosecutor flags the word linked"), the user corrected the house voice. The persona is the ideological framework the article is reported through. It saturates the structure — which numbers get highlighted, which incentives get named, which weapons get mapped — but it never walks on stage. A reporter writing with an economist's instincts says "the ratio here is 50 to 1" and lets the reader feel the lens in the metric choice. They do not write "The Economist sees a 50 to 1 ratio." The subheadline and summary can still reference the persona (those are editorial chrome around the article), but the body and callouts stay in news voice. Grep the JSON for any of the 29 persona strings (`The Hawk`, `The Dove`, `The Prosecutor`, `The Economist`, `The Historian`, `The Mirror`, `The General`, `The Futurist`, `The Populist`, `The Cynic`, `The Realist`, `The Peacekeeper`, `The Technocrat`, `The Freeman`, `The Idealist`, `The Judge`, `The Contrarian`, `The Logician`, `The Scholar`, `The Synthesizer`, `The Visionary`, `The Revolutionary`, `The Traditionalist`, `The Elitist`, `The Evangelist`, `The Everyman`, `The Gaslighter`, `The Operator`, `The Politician`) before the INSERT. If any match, mark the row failed and start the paragraph over. Lowercase generic labels (`hawks`, `doves`, `economists`, `realists`) are fine because they are standard English political vocabulary, not named characters.

**The double-apostrophe gate catches a Postgres escaping bug.** Inside `$json$...$json$` dollar-quoted blocks, apostrophes are literal — do NOT double them. Writing `Iran''s` inside `$json$` stores the literal two-character sequence `''` which in proportional fonts looks like a double quote. Only regular `'...'` single-quoted SQL string literals use `''` to escape an apostrophe. If the writer catches the mistake before the INSERT, fix it in place. If it is caught after, run the repair: `UPDATE news_reports SET body = REPLACE(REPLACE(body::text, $$''''$$, $$'$$), $$''$$, $$'$$)::jsonb, callouts = REPLACE(REPLACE(callouts::text, $$''''$$, $$'$$), $$''$$, $$'$$)::jsonb WHERE slug = ...;`

**Skipping any gate is not an option.** If generate-article.md proceeds toward step 7's INSERT without you having checked all 13 gates above, stop it and mark the row `failed` with `gates_skipped: pipeline reached INSERT without quality validation`. These checks are the contract; the contract is the brand.
