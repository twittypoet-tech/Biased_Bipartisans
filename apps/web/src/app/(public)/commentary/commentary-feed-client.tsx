    ...
    <article className="rounded-xl border border-t-edge bg-t-surface shadow-t overflow-hidden">
      ...
      {c.report_slug && (
        <Link href={`/reports/${c.report_slug}`} className="group block">
          <div className={`px-4 py-2 flex items-center gap-2 transition hover:opacity-90 ${c.report_category ? CATEGORY_BANNER[c.report_category] ?? 'bg-t-surface-el text-t-text-3' : 'bg-t-surface-el text-t-text-3'}`}>
